...
# Custom CSS for a condensed, information-dense "Glassmorphism" UI
st.markdown("""
    <style>
    ...
    /* Compact Sentiment/Intelligence Box */
    .intel-box {
        padding: 15px;
        border-radius: 10px;
        color: white;
...
def calculate_performance(df):
    ...
    return {
        "1W": get_pct_change(5),
        "2W": get_pct_change(10),
        "1M": get_pct_change(21),
        "2M": get_pct_change(42)
    }

# --- USER INTERFACE ---

def main():
    # Integrated Header and Top Controls
    t_col1, t_col2, t_col3, t_col4 = st.columns([1.5, 1.2, 0.8, 0.5])
    
    with t_col1:
        st.title("🛰️ Market Weather Station")
    
    with t_col2:
        selected_display = st.selectbox(
            "Monitoring Target",
            options=list(PRESET_COMPANIES.keys()),
    ...
    with t_col3:
        data_range = st.selectbox(
            "History Depth", ["1y", "2y", "5y"], 
            index=1, label_visibility="collapsed"
        )

    with t_col4:
        auto_refresh = st.checkbox("Live Pulse", value=True)
    ...
    if target_ticker:
        ...
        if df is not None and len(df) > 10:
            ...
            with m_col1:
                ...
                currency_symbol = "₹" if any(suffix in target_ticker for suffix in [".NS", ".BO", "^NSEI"]) else "$"
                
                s_col1.metric("Live Price", f"{currency_symbol}{curr_price:,.2f}", f"{delta_price:.2f}%")
                
                pred, prob, importance = train_ml_model(df)
                s_col2.metric("ML Forecast", "☀️ BULLISH" if pred == 1 else "🌧️ BEARISH")
                ...
                def format_perf(val, label):
                    color = "#3fb950" if val >= 0 else "#f85149"
                    prefix = "+" if val >= 0 else ""
                    return f"""
                    <div class="perf-card">
                        <div class="perf-label">{label}</div>
                        <span style="color:{color}; font-weight:700; font-size:1.1rem;">{prefix}{val:.2f}%</span>
                    </div>
                    """
                p_col1.markdown(format_perf(perf["1W"], "1W"), unsafe_allow_html=True)
                p_col2.markdown(format_perf(perf["2W"], "2W"), unsafe_allow_html=True)
                p_col3.markdown(format_perf(perf["1M"], "1M"), unsafe_allow_html=True)
                p_col4.markdown(format_perf(perf["2M"], "2M"), unsafe_allow_html=True)

            with m_col2:
                # Intelligence/Sentiment Summary
                sentiment_score = (prob - 0.5) * 2 
                color = "rgba(40, 167, 69, 0.8)" if sentiment_score > 0 else "rgba(220, 53, 69, 0.8)"
                st.markdown(f"""
                    <div class="intel-box" style="background-color: {color};">
                        <div style="font-size:0.7rem; opacity:0.8; text-transform:uppercase;">Momentum Vibe</div>
                        <h3 style="color: white; margin: 0; font-size: 1.4rem;">{"HEALTHY" if sentiment_score > 0 else "CAUTION"}</h3>
                        <div style="font-size:0.7rem; margin-top:5px;">Intensity: {abs(sentiment_score):.2f}</div>
                    </div>
                    """, unsafe_allow_html=True)

            if auto_refresh:
                st.markdown(f'<span class="live-pulse">● {comp_name} LIVE</span>', unsafe_allow_html=True)
            ...
            with c_col2:
                ...
                fig_imp = go.Figure(go.Bar(
                    x=imp_df['Weight'], y=imp_df['Factor'], orientation='h',
                    marker_color='#58a6ff'
                ))
                fig_imp.update_layout(template="plotly_dark", height=200, margin=dict(l=0, r=0, t=10, b=0),
                                     paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                                     xaxis=dict(showticklabels=False), yaxis=dict(tickfont=dict(size=10)))
                st.write("Neural Weighting")
                st.plotly_chart(fig_imp, use_container_width=True)
                
                with st.expander("Sensor Logs"):
